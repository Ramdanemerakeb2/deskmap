...
public class OverPassNode implements Serializable {
	...
	public static OverPassNode xyToNode(double x, double y, double centerLon, double centerLat, double scale) {
        OverPassNode node = new OverPassNode(centerLon, centerLat);
        System.out.println("MeterE "+((-Map.mapDimension/2 + x)*scale)+" MeterN "+(Map.mapDimension/2 - y)*scale);
        System.out.println("XXMeterE "+node.est+" MeterN "+node.nord);
        node.add((-Map.mapDimension/2 + x)*scale, (Map.mapDimension/2 - y)*scale);
        return node;
	}
	...
	public double[] getScreenPos(double scale, OverPassNode limitTop) {
		limitTop = Map.getMapInstance().limitTop;
	...
	public double getScreenDistance(OverPassNode node) {
		double[] xy = getScreenPos(Map.getMapInstance().scale, Map.getMapInstance().limitTop);
		double[] xy2 = node.getScreenPos(Map.getMapInstance().scale, Map.getMapInstance().limitTop);
